...
int main(int argc, char ** argv)
{
    ...
    /* 必须在SelectAndInitDisplay后调用AllInputDevicesInit*/
	...
	while (1)
	{
		...
		{
    		if (tInputEvent.code == INPUT_EVENT_DOWN)
    		{
    			ShowNextPage();
    		}
    		else if (tInputEvent.code == INPUT_EVENT_UP)
    		{
    			ShowPrePage();			
    		}
    		else if(tInputEvent.code == INPUT_EVENT_EXIT)
    		{
    			return 0;
    		}	
        }
